...
response = WS.sendRequest(findTestObject('EP_Booking/Get Booking Detail by Id'))
...
//
//bookingCheckIn = result.bookingdates.checkin
//
//bookingCheckOut = result.bookingdates.checkout
//
	...
	if(lastNameVerified == true) {
		...
		if(totalPriceVerified == true) {
			...
			if(depositPaidVerified == true) {
				...
				WS.comment("The item with Deposit Pait $depositPaid is invalid")
			}
		}else {
